 ...
 */
async function transformMasonryHtml(str, data) {
    const theme = hexo.theme.config;
...
hexo.extend.filter.register(
  "after_render:html",
  transformMasonryHtml,
  15, // Run before other after_render:html filters (img-optimizer runs at default priority)
);

// Exposed for HTML that never becomes a route and so never reaches the filter
// above — the sealed page of an encrypted masonry album. Without it the album's
// images stay plain <img> tags and skip the whole lazy pipeline.
hexo.extend.helper.register("lazyloadMasonryHtml", transformMasonryHtml);
